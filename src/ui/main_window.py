...
from src.utils.setup_handler import handle_ui_texts
from src.utils.ui_helpers import get_current_filter
...
class MainWindow(QMainWindow):
    ...
    def create_connection(self) -> None:
        ...
        self.processing_widget.start_button.clicked.connect(self.process_controller.start_process)
    ...
    def handle_dialog_result(self, path: list[str] | str, action: str) -> None:
        if action in ["input_folder_processing_widget", "add_files_processing_widget"]:
            if isinstance(path, list):
                for str_pah in path:
                    new_path = pathlib.Path(str_pah)
                    if new_path not in self.process_controller.selected_files:
                        self.process_controller.selected_files.add(new_path)
                        self.processing_widget.update_count_labels(files_count=1)
